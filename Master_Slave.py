...
def master_slave():
    ...
    list = base.PickEntities(
        deck, prop, filter_visible=True, prop_from_entities=True)
    if list == None:
        error()
        return 1
...
def error():
    window = guitk.BCMessageWindowCreate(
        guitk.constants.BCMessageBoxWarning, 'No element selected', False)
...
def table(master_list, slave_list):
    window = guitk.BCWindowCreate(
        "Master Slave Table", guitk.constants.BCOnExitDestroy)
    table = guitk.BCTableCreate(window, 2, 2)
    guitk.BCTableSetNumRows(table, len(master_list))
    guitk.BCTableHeaderSetLabel(
        table, guitk.constants.BCHorizontal, 0, 'Master')
    guitk.BCTableHeaderSetLabel(
        table, guitk.constants.BCHorizontal, 1, 'Slave')
    i = 0
