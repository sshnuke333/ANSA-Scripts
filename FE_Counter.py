...
duplicate materials.
	...
	window1 = guitk.BCWindowCreate("Select PID Format", guitk.constants.BCOnExitDestroy)
	guitk.BCWindowShowTitleBarButtons(window1, guitk.constants.BCMinimizeButton | guitk.constants.BCMaximizeButton)
	button1 = guitk.BCPushButtonCreate(window1, "Partname to PID", PPID,window1)
	button2= guitk.BCPushButtonCreate(window1, "Filename to PID", FPID,window1)
...
def main(name):
	...
	BCBoxLayout_1 = guitk.BCBoxLayoutCreate(window2, guitk.constants.BCHorizontal)
...
def _OK(window2,data):
	...
	for file in list_of_files:
		if file.endswith('.ansa'):
			...
			pids = base.CollectEntities(deck,None,prop,filter_visible=True,prop_from_entities=True)
			...
			for j in range(0,len(t_ascend)):
				...
				# set PID number to random 135792468 to avoid error
				if name == FPID:
					number = actual_name[0].count("/")
					temp_name= actual_name[0].split("/")
					clean_name = temp_name[number]
					ent.set_entity_values(deck,{'PID':135792468})
					ent.set_entity_values(deck,{"Name":clean_name +'_'+str(thickness)+'mm','PID':COUNTER})
				elif name == PPID:
					k = 0
					...
					for parts[k] in parts:
						...
						if a.__contains__(b) == True:
							...
							ent.set_entity_values(deck,{"Name":part_name+'_'+str(thickness)+'mm','PID':135792468})
			...
			for j in range(0,len(t_ascend)):
				...
				thickness = t_ascend[j]
				ent.set_entity_values(deck,{"FROZEN_ID":'NO'})
				ent.set_entity_values(deck,{"T1": thickness,'PID':10000000})
				pidn = ent.get_entity_values(deck,{'PID'})
				if name == FPID:
					number = actual_name[0].count("/")
					temp_name= actual_name[0].split("/")
					...
					for parts[k] in parts:
						...
						if a.__contains__(b) == True:
							part_name = parts[k]._name
							ent.set_entity_values(deck,{"Name":part_name+'_'+str(thickness)+'mm','PID':COUNTER})
							break
						else:
							k +=1
				COUNTER +=1
...
def Checks(deck,prop):
	...
	user_thic = base.CheckAndFixPenetrations(type=3,fast_run=False,fix=False,user_thic=0.6)
